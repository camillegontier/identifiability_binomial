...
def EM_Binomial(e,N,p_init,q_init,sigma_init):
    ...
    for it in range(max_it):
        ...
        for kk in range(N+1):
            for ii in range(T):
                if kk == 0:
                    p_k_e[kk,ii] = float(e[ii]==0)*scipy.stats.binom.pmf(kk,N,p_old)                    
                elif e[ii] != 0:
                    p_k_e[kk,ii] = (q_old**(3/2)*kk/np.sqrt(2*np.pi*sigma_old**2*e[ii]**3))*np.exp(-(q_old*(e[ii]-q_old*kk)**2)/(2*sigma_old**2*e[ii]))*scipy.stats.binom.pmf(kk,N,p_old)
        ...
        num = np.sum(np.sum(e[i]*p_k_e[ki,i]/p_e[i] for ki in range(N+1)) for i in range(T))
        den = np.sum(np.sum(ki*p_k_e[ki,i]/p_e[i] for ki in range(N+1)) for i in range(T))   
        q_new = num/den
        ...
        for ki in range(N+1):
            for i in range(T):
                if e[i] != 0:
                    num = num + ((e[i]-q_new*ki)**2/e[i])*p_k_e[ki,i]/p_e[i]
