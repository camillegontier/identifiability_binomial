# -*- coding: utf-8 -*-
"""
Created on Tue Sep 17 11:08:42 2019

@author: gontier
"""

# Computes the log likelihood of data under model M_3
...
def ll_binomial_tau_D_tau_F(EPSP,N,p,q,sigma,tau_D,tau_F,delta_t):
    T = EPSP.size
        ...
        for k1 in range(n1+1):
            for n2 in range(N+1):
                for k2 in range(n2+1):
                    u = p
                    for i in range(T-1):
                        u = p + u*(1-p)*np.exp(-delta_t[i+1]/tau_F)
                        A[n1,k1,n2,k2,i+1] = scipy.stats.binom.pmf(k2,n2,u)*scipy.stats.binom.pmf(n2-n1+k1,N-n1+k1,1-np.exp(-delta_t[i+1]/tau_D))
