...
def hessian(e,N,p,q,sigma,tau,delta_t):
    ...
    for n in range(N+1):
        for k in range(n+1):
            for i in range(T):
                B[n,k,i] = scipy.stats.norm.pdf(e[i],loc=q*k,scale=sigma)
                
                dB_dq[n,k,i] = B[n,k,i]*k*(e[i] - q*k)/sigma**2
                
                dB_dsigma[n,k,i] = B[n,k,i]*((e[i]-q*k)**2/sigma**3 - 1/sigma)
                
                dB_dqdq[n,k,i] = dB_dq[n,k,i]*k*(e[i] - q*k)/sigma**2 + \
                B[n,k,i]*-(k/sigma)**2
                
                dB_dsigmadsigma[n,k,i] = dB_dsigma[n,k,i]*((e[i]-q*k)**2/sigma**3 - 1/sigma) + \
                B[n,k,i]*(1/sigma**2-3*(e[i]-q*k)**2/sigma**4)
                
                dB_dqdsigma[n,k,i] = dB_dsigma[n,k,i]*k*(e[i] - q*k)/sigma**2 + \
                B[n,k,i]*-2*k*(e[i]-q*k)/sigma**3
